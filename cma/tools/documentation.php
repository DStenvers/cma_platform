...
DEPLOY_SECRET=&lt;64-char hex via openssl rand -hex 32&gt;</code></pre>
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td><code>.sessions/</code></td><td>PHP-session files (een dot-map; een bestaande <code>sessions/</code>-map wordt bij de eerste bootstrap automatisch hernoemd). Opschoning: PHP's eigen GC draait gegarandeerd 1-op-100 per <code>session_start</code> en verwijdert files ouder dan <code>session.gc_maxlifetime</code>. De map hoort in <code>hiddenSegments</code> (zie de check hierboven). Een open sessie houdt een exclusieve lock op haar bestand tot het einde van het verzoek; <code>form_api.php</code> geeft die direct na de bootstrap vrij (<code>session_write_close</code>), omdat een formulier via meerdere gelijktijdige aanroepen laadt (init, keuzelijsten, boom; record en subformulieren) die anders achter elkaar in de rij stonden — gemeten: acht gelijktijdige aanroepen 185-337 ms elk mét lock, 119 ms zonder. Het endpoint leest de sessie alleen; wie er in een API wél iets in wil schrijven, opent haar opnieuw met <code>session_start()</code>.</td></tr>
...
function render_doc_deployment(): void
{
    ...
    <ol>
        ...
        <li><span class="cma-tool__strong">Composer update</span> — <code>composer update &lt;DEPLOY_COMPOSER_UPDATE&gt; --no-dev --optimize-autoloader</code> zodat het platform-package mee-loopt. Composer-binary wordt over meerdere bekende paden geprobeerd (app-pool-identity heeft het zelden op PATH). Als <code>DEPLOY_COMPOSER_CLEAR_CACHE=1</code> staat (of de deploy-URL <code>?forcerefresh=Y</code> bevat) draait er eerst een <code>composer clear-cache</code>, zodat een net-gepushte platform-tag niet wordt gemist door een stale VCS-clone-cache. Andersom: een deploy die alleen consumer-/front-end-bestanden raakt (geen platform-update nodig) kan de héle composer-stap overslaan met <code>?nocomposer=Y</code> op de deploy-URL — git pull, test-gate, asset-build en recycle draaien gewoon door, dus zo'n deploy is in seconden klaar i.p.v. wachten op <code>composer update</code>. <span class="cma-tool__strong">De draaiende platformversie staat altijd in het log</span> — vóór de stap (<code>platform vooraf:</code>), erna (<code>platform na:</code>, met <code>(ONGEWIJZIGD)</code> als er niets bewoog) én in de footer van elke run. Dat staat er ook als de composer-stap wordt overgeslagen, zodat "welk platform draait hier" een kwestie van kijken is — van buitenaf via <code>deploy_status.php</code>, zonder servertoegang. Na afloop vergelijkt <code>DeployHealth::platformVersionCheck()</code> de commit uit <code>composer.lock</code> met die in <code>vendor/composer/installed.json</code>; lopen ze uiteen, dan heeft de composer-stap niet gedaan wat de deploy aannam en volgt er een <code>LOOPT ACHTER</code>-regel plus een best-effort alert-mail. Let op wat die controle niet kan zien: staan lock én vendor allebei op een oude release, dan kloppen ze met elkaar en zwijgt hij — dáárvoor is die <code>platform:</code>-regel.</li>
...
function render_doc_routing(): void
{
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td><code>form.php</code></td><td>Zet <code>data-record-id</code> + body-classes (<code>has-record</code>, <code>mode-tree/table</code>) op basis van de route. De controller laadt het record náást de lijst. In nomenu-modus gaat alleen de body-inhoud naar de shell, voorafgegaan door de config-scripts uit de <code>&lt;head&gt;</code> (<code>FormTemplate::headScriptsForShell</code>); het init-script aan het eind van de body zit al in die inhoud en mag niet ook vooraf worden uitgestuurd, anders ontstaan twee controllers die elk de lijst, de boom en de keuzelijsten ophalen.</td></tr>
        </tbody>
    </table>
...
JsonFormLoader::setFileCacheEnabled(false);               // disable disk-cache
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td><code>postHandler</code> / <code>afterPostUrl</code> / <code>previewUrl</code> / <code>onLoadJs</code></td><td>Hooks: eigen POST-handler PHP-bestand; <code>afterPostUrl</code> = pagina die na elke save én delete gePOST wordt met alle velden, hun <code>_old_*</code>-waarden, de changelog en <code>_changelog_type</code> (add/edit/delete), <code>formid</code>, <code>recid</code>, <code>table</code> &mdash; de database-trigger die Access niet kent (<code>../cma_afterpost.asp</code> wordt <code>/cma_afterpost.php</code>); preview-URL-template; on-load JavaScript.</td></tr>
            <tr><td><code>notificaties</code> (gebruikers)</td><td>Wie op het gebruikersformulier bij <em>Notificaties</em> een formulier aanvinkt, krijgt na elke toevoeging, wijziging of verwijdering in dat formulier een e-mail met de wijzigingen (los van de monitoring-instelling); met <em>Geen notificaties eigen wijzigingen</em> niet voor eigen acties. De oude <em>Data meldingen</em> (verlopen XML-gegevensbronnen, <code>tblUserDataNotifications</code>, dagelijkse taak) zijn vervallen: het platform heeft geen XML-store-runtime, dus de conditie "bron raakt over N dagen leeg" is niet te bepalen; <code>task.php</code> mailt daar niet meer voor.</td></tr>
            <tr><td><code>allowDelete</code></td><td>Verwijderen is definitief (<code>DELETE</code>). Er is geen soft delete: het klassieke <code>DeletedField</code> is nooit gebruikt en niet overgenomen; wie een record wil bewaren markeert het via een eigen veld (bijv. <code>actief</code>) en filtert daarop.</td></tr>
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td><code>caption</code> / <code>captionEnglish</code></td><td>Label (NL / EN). Ontbreekt de caption, dan valt de weergave terug op de ruwe veldnaam; bekende afkortingen uit geconverteerde databases worden daarbij vertaald (<code>JsonFormService::fieldNameCaption</code>, o.a. <code>descr</code> → "Omschrijving") — breid die map uit als er meer opduiken. Als kolomkop in de lijst en in subformulieren telt alleen het deel vóór een <code>&lt;br&gt;</code>, zonder tags (<code>JsonFormService::columnTitle</code>): een meerregelige caption boven een invoerveld blijft dan een korte kop boven een kolom.</td></tr>
    ...
    <p>Die maat komt pas als de balk er is. Zolang er geen record is, staat hij verborgen: op de lijstpagina tot je in de boom een record kiest, op de toevoegpagina tot het opslaan. Op een verborgen balk zet <code>cma-fold</code> niets (zie hieronder); hij houdt zichzelf met een <code>ResizeObserver</code> in de gaten en zet de bewaarde of standaardmaat zodra hij zichtbaar wordt. Zonder die stap heeft de sectie in de boomweergave geen maat, wordt ze zo hoog als de lijst in het actieve tabblad, verandert ze bij elke tabwissel van hoogte en drukt ze het formulier erboven weg.</p>
    ...
    <p>Startwaarden komen uit de definitie: een veld met <code>"defaultValue"</code> krijgt van de renderer een <code>data-default</code>-attribuut, en <code>applyDefaultValues()</code> zet dat op de besturing zodra het lege formulier klaarstaat. Bij een schakelaar is dat de enige manier waarop hij aan opent — de server rendert een <code>&lt;lib-switch&gt;</code> nooit met <code>checked</code>. Er lopen <span class="cma-page__strong">twee</span> wegen naar een leeg formulier en ze hebben die aanroep allebei nodig: <code>newRecord()</code> (de knop Toevoegen in een al geladen formulier) en de init-tak voor <code>is-creating</code> — een server-gerenderd nieuw record, dus <code>/cma/form/&lt;form&gt;/new</code> en het zijpaneel dat die URL opent. Ontbreekt hij op één van beide, dan opent hetzelfde formulier langs de ene weg mét en langs de andere zónder zijn startwaarden. De toepassing gaat vóór <code>setParentFieldValue()</code> en <code>setFilterFieldValue()</code>, zodat een parent- of filterwaarde een startwaarde overschrijft en niet andersom. <code>cma/tests/js/nieuwrecord-startwaarden.test.js</code> houdt beide wegen vast.</p>
...
function render_doc_web_components(): void
{
    ...
    <table class="listtable">
        ...
        <tbody>
            <tr><td><code>lib-*</code></td><td><code>library/webcomponents/</code></td><td>Reusable building blocks zonder CMA-specifieke afhankelijkheden. Voorbeelden: lib-dialog, lib-combo, lib-datepicker, lib-sheet, lib-pagination. Ook bruikbaar in front-end pages van consumer-sites.</td></tr>
    ...
    <p><span class="cma-tool__strong">Dubbelklik op de kop</span> zet alles terug naar de standaard en wist de bewaarde stand. Die standaard (boven, rechts, onder, breedte) staat alleen als inline stijl op het paneel, gezet bij het openen; het stylesheet kent geen plek of breedte voor de container. De code onthoudt die waarden daarom vóór een bewaarde stand ze overschrijft en zet ze bij de dubbelklik terug — alleen de inline stijlen wissen laat een <code>position:fixed</code>-paneel zonder plek achter, buiten beeld. Dat is ook de uitweg als een paneel ooit buiten beeld belandt; los daarvan klemt de code de positie altijd zo dat er minstens 120px zichtbaar blijft.</p>
...
function render_doc_troubleshooting(): void
{
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td>Storybook, sectie libLog: <code>libLog.flush is not a function</code> (ook getRequestId, isDebug)</td><td>lib-log.js zit bewust niet in de bundel en de storybook laadde het ook niet los; de sectie viel terug op de kale libLog uit library.js, die alleen log/warn/info/error kent.</td><td>Sinds 1.38.53 laadt storybook.php lib-log.js zelf, met een absoluut pad (<code>/library/…</code>: de pagina staat in /cma/tools/, dus <code>../library/…</code> gaf een 404). Test: <code>cypress/e2e/tools/storybook-liblog.cy.js</code>.</td></tr>
            <tr><td>Een lijst met bladerknoppen: elke site tekende zijn eigen pager</td><td>Er was geen component voor; de CMA-lijsten pagineren met infinite scroll in form-controller.js en dat is niet los te gebruiken.</td><td>Sinds 1.38.54 is er <code>&lt;lib-pagination&gt;</code> (library/webcomponents): links via een href-sjabloon met <code>{page}</code> voor server-side bladeren, of het event <code>page-change</code> voor client-side; thema via CSS-variabelen. Zie de storybook. Tests: <code>cma/tests/js/pagination.test.js</code>, <code>cypress/e2e/components/lib-pagination.cy.js</code>.</td></tr>
    ...
    <p class="docs-meta">De JSON-form lijst laadt bij >500 rijen de eerste pagina server-side en haalt de rest op de achtergrond op in batches van 500 (keyset-paginering op het unieke id-veld), tot <code>LIST_PREFETCH_MAX_ROWS</code> rijen (Systeeminstellingen → Lijsten, standaard 1000); wat daarboven zit laadt bij doorscrollen. Die grens is geen luxe: elke bijgeladen stap kost meer dan de vorige, want de rijtelling en de kolomfilters lopen over de hele tabel. Gemeten op een lijst van 53.000 rijen: na 45 seconden 6.400 rijen, 200.000 DOM-knopen en 40 seconden geblokkeerde hoofdthread, en het zou zes minuten zijn doorgegaan. Om dezelfde reden herbouwt het achtergrond-laden de kolomfilters niet per stap maar één keer aan het eind (<code>CmaInfiniteScroll.refreshFilters()</code>); een scroll-stap herbouwt ze wel meteen. Access kent geen <code>LIMIT/OFFSET</code>, alleen <code>TOP</code> — daarom keyset (<code>WHERE [id] &gt; lastId ORDER BY [id]</code>) i.p.v. een offset-lus; dát is de reden dat het "gewoon een <code>while(recordsLeft)</code>-lus" al is (<code>_autoPrefetchRows()</code>) maar niet op offset kan draaien. De teller in de toolbar (<code>records 1-N van M</code>, met suffix <code>(laden…)</code> zolang <code>hasMore</code>) is puur een voortgangsindicator: is alles binnen, dan verdwijnt hij. Kern: <code>CmaInfiniteScroll</code> in <code>cma/assets/js/table-preferences.js</code>, aangedreven door <code>_autoPrefetchRows()</code> in <code>cma/assets/js/form-controller.js</code>.</p>
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td>Console-<span class="cma-tool__em">error</span> (unhandledrejection, foutpaneel, serverrapport) <code>[Infinite Scroll] Pagination stopped at 2144/2304 — 160 record(s) not loaded (last id 2392, form logins)</code>, terwijl de teller eerlijk <code>1-2144 van 2304</code> toont en de lijst gewoon bruikbaar is</td><td>Niet de paginering maar de <span class="cma-tool__strong">ernst</span> van de melding. <code>load()</code> gooide een Error zodra de paginering eindigde onder het bekende totaal, om een stille korte stop zichtbaar te maken. Maar zo'n stop is geen fout: de tellerstand vertelt al wat er ontbreekt, en de gebruiker kan verder.</td><td>Sinds 1.38.50 is het een <code>cmaLog.warn</code> met dezelfde tekst en tellerstand, geen Error meer. Hij zwijgt nog steeds bij een onderbreking (pagina verlaten, tabel uit de DOM) omdat de tellerstand dan niets zegt. Test: <code>cma/tests/js/infinitescroll-onderbroken.test.js</code>.</td></tr>
            ...
            <tr><td>Een grote lijst (duizenden rijen) blokkeert de browser minutenlang na het openen; scrollen en klikken reageren pas na lange tijd</td><td>Het achtergrond-laden haalde álle rijen op, en elke stap werd trager dan de vorige (rijtelling en kolomfilter-herbouw over de hele tabel).</td><td><code>LIST_PREFETCH_MAX_ROWS</code> begrenst het achtergrond-laden (standaard 1000 rijen; 0 = nooit op de achtergrond), daarna laadt scrollen verder. Staat de grens hoger, zet hem terug.</td></tr>
