...
function render_doc_troubleshooting(): void
{
    ...
    <p class="docs-meta">De JSON-form lijst laadt bij >500 rijen de eerste pagina server-side en haalt de rest op de achtergrond op in batches van 500 (keyset-paginering op het unieke id-veld). De teller in de toolbar (<code>records 1-N van M</code>, met suffix <code>(laden…)</code> zolang <code>hasMore</code>) is puur een voortgangsindicator: is alles binnen, dan verdwijnt hij. Kern: <code>CmaInfiniteScroll</code> in <code>cma/assets/js/table-preferences.js</code>, aangedreven door <code>_autoPrefetchRows()</code> in <code>cma/assets/js/form-controller.js</code>. De server (<code>JsonFormService::getTableHtml</code>) is bewezen schoon — tabelweergave gebruikt de unieke id-keyset, geen dubbele/overgeslagen rijen; een blijvende <code>(laden…)</code> is altijd client-side.</p>
    <table class="listtable">
        <thead><tr class="listheader"><th style="width:340px">Symptoom</th><th>Oorzaak</th><th>Fix</th></tr></thead>
        <tbody>
