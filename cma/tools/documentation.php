...
function render_doc_web_components(): void
{
    ...
    <p><code>cma-launcher</code> is de BIG-menu overlay achter de <span class="cma-tool__strong">Menu</span>-knop op de tools-pagina. Het haalt de tools-catalogus op via <code>api/tools-catalog.php</code> (JSON uit <code>buildToolsTreeData()</code> in <code>cma/tools_catalog.inc</code> — dezelfde bron als het tools-menu) en toont die als doorzoekbaar, gegroepeerd raster. Een keuze laadt via <code>window.loadPage()</code> in <code>#contentArea</code> (form-backed items &rarr; <code>/cma/form/&lt;form&gt;</code>, tool-pagina's &rarr; <code>tools.php?tool=&lt;naam&gt;</code>), zodat header + sidebar blijven staan. Het paneel vult <code>#contentArea</code> op 100% (absoluut gepositioneerd binnen <code>.cma-main</code>, onder de header) i.p.v. een viewport-brede dropdown — header en sidebar blijven zichtbaar en bruikbaar. Sinds v1.28.45 krijgt het item van de tool/het formulier dat op dat moment open staat de klasse <code>.cma-launcher__item.is-active</code> (gemarkeerd via <code>_markActive()</code> op basis van de <code>?tool=</code>-sleutel of het <code>/cma/form/&lt;form&gt;</code>-pad). Light DOM; stijlen staan als <code>.cma-launcher__*</code> in <code>assets/css/main.css</code>.</p>
