...
function render_doc_web_components(): void
{
    ...
    <ul>
        <li><span class="cma-tool__strong"><code>nav-mode="shell"</code></span> (default) — het tools-menu achter de <span class="cma-tool__strong">Menu</span>-knop in de shell-header. Haalt de catalogus uit <code>api/tools-catalog.php</code> (<code>buildToolsTreeData()</code> in <code>cma/tools_catalog.inc</code>), toont als overlay die <code>#contentArea</code> op 100% vult (absoluut binnen <code>.cma-main</code>, onder de header), en navigeert via <code>window.loadPage()</code> + schone URL (form-items &rarr; <code>/cma/form/&lt;form&gt;</code>, tools &rarr; <code>tools.php?tool=&lt;naam&gt;</code>). Header + sidebar blijven zichtbaar en bruikbaar.</li>
        <li><span class="cma-tool__strong"><code>nav-mode="iframe"</code></span> (sinds v1.28.99, gebruikt door <code>reports.php</code>) — een <span class="cma-tool__em">embedded</span> menu dat zijn eigen host-box (<code>#reports</code>) op 100% vult i.p.v. de hele <code>#contentArea</code>. Haalt de catalogus uit de opgegeven <code>catalog-url</code> (<code>api/reports-catalog.php</code>) en laadt bij een keuze het <code>href</code> van het item in het iframe genoemd door <code>target</code> (<code>#reports-content</code> &rarr; <code>reportdetails.php?RepID=X</code>), verbergt zichzelf en schrijft het schone <code>url</code> naar de adresbalk. Geen backdrop, geen body-scroll-lock. De host is "leeg of een menu": open menu &rarr; iframe verborgen; keuze/sluiten &rarr; iframe zichtbaar als er een rapport geladen is.</li>
    </ul>
    ...
    <p><code>/cma/tools</code> toont een smalle <code>cma-toolbar</code> met één <span class="cma-tool__strong">Menu</span>-knop (<code>#toolsMenuBtn</code>) in het content-gebied. Sinds v1.28.45 is dit het enige entree-punt: de vroegere header-knop (<code>#menuLauncherBtn</code> in <code>main.php</code>, v1.28.39–1.28.44) is verwijderd. De knop <span class="cma-tool__strong">togglet</span> de gedeelde launcher (<code>window.toggleToolsLauncher()</code>) — nogmaals klikken sluit hem. Zonder <code>?tool=</code> opent de launcher automatisch (<code>window.openToolsLauncher()</code>, met korte retry); de tekst-hint (<code>.tools-empty</code>) verschijnt alleen als fallback wanneer de shell-launcher onbereikbaar is. Een keuze laadt de tool full-width in <code>#tools-content</code>. De voormalige twee-paneel boomstructuur is gearchiveerd als <code>cma/tools_DEPRECATED.php</code>.</p>
