...
function render_doc_troubleshooting(): void
{
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td>Veld is onzichtbaar én onbewerkbaar; opslaan bewaart steeds de oude waarde (console: 500/404 op <code>contentblocks.json</code>)</td><td>De bloktemplates (<code>contentblocks.json</code>) konden niet geladen worden, dus blockedit rendert geen blokken en <code>blockedit_collect_htmls</code> serialiseert bewust niets (de v1.20.19 loss-prevention bewaart dan de oude veldwaarde i.p.v. leeg op te slaan). De hoofdveld-editor is door CSS tot 0px ingeklapt, dus de operator ziet niets en kan niets bewerken.</td><td>v1.26.3: loader probeert meerdere locaties (<code>/cma/assets/contentblocks/contentblocks.json</code>, dan <code>/cma_contentblocks.json</code> voor oudere CMA's). v1.26.4: falen álle locaties, dan degradeert het veld naar gewone bewerking — de hoofdveld-editor wordt uitgeklapt (of de ruwe textarea getoond) zodat inhoud zichtbaar en bewerkbaar blijft. De échte fix is altijd server-side: controleer waarom de JSON-URL faalt (rewrite-rule, rechten, ontbrekend bestand).</td></tr>
        </tbody>
    </table>
