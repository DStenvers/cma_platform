    ...
    <pre><code>curl 'https://&lt;host&gt;/deploy_status.php?config=1'
{
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td><code>DEPLOY_COMPOSER_CLEAR_CACHE</code></td><td><lib-label type="information">nee</lib-label></td><td><code>(uit)</code></td><td><code>1</code> = draai <code>composer clear-cache</code> VÓÓR de update. Nodig wanneer een net-gepushte platform-tag niet wordt opgepikt omdat Composer's VCS-clone-cache een oude ref vasthoudt (&ldquo;Nothing to modify&rdquo;). Uit by default (re-clone is trager); zet aan als een deploy een gloednieuwe release moet trekken. Best-effort: een falende clear-cache breekt de deploy niet af.</td></tr>
...
function render_doc_troubleshooting(): void
{
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td>Net-gereleasete platform-versie wordt niet opgepikt ("Nothing to modify"), <code>composer.lock</code> blijft op de oude versie</td><td>Composer's VCS-clone-cache houdt een oude ref vast; de nieuwe tag wordt niet gezien.</td><td>Zet <code>DEPLOY_COMPOSER_CLEAR_CACHE=1</code> in <code>.env</code> zodat de deploy eerst <code>composer clear-cache</code> draait. Handmatig: <code>composer clear-cache &amp;&amp; composer update stenversonline/platform</code>. Niet <code>--no-cache</code> gebruiken (lost de stale clone niet op).</td></tr>
