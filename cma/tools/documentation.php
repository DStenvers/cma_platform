...
/images/.responsive/foto.webp         &larr; volledige WebP</code></pre>
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td>Batch-conversie stopt na een paar beelden met HTTP 500</td><td>Grote afbeelding put GD-geheugen uit → niet-vangbare fatal breekt de request af</td><td>Sinds v1.28.116 slaat de tool te grote beelden over met reden en meldt het bestand waarop een fatal stukliep op het scherm. Verhoog <code>memory_limit</code> of verklein het bronbeeld.</td></tr>
...
&lt;/outboundRules&gt;</code></pre>
    ...
    <p><span class="cma-tool__strong">Gecompileerde Twig-templates</span>: sinds v1.29.123 leegt <a href="tools.php?tool=clearcache" target="_top">Cache leegmaken</a> ook <code>.cache/twig</code> (recursief) en toont het die als eigen regel <span class="cma-tool__strong">Twig</span> — maar alléén op sites waar die map bestaat. Twig's <code>auto_reload</code> hercompileert normaal vanzelf bij een gewijzigde <code>.twig</code>-mtime; je hebt de handmatige clear nodig na een deploy die templates met een óudere mtime terugzet, of na een wijziging in de Twig-opties, filters of globals — die worden namelijk in de gecompileerde PHP-klasse gebakken.</p>
...
function render_doc_troubleshooting(): void
{
    ...
    <table class="listtable">
        ...
        <tbody>
            ...
            <tr><td><code>Installer::postUpdate</code> stopt met <code>copy(... desktop.ini): Permission denied</code></td><td>Een Windows-junk-bestand (<code>desktop.ini</code> / <code>Thumbs.db</code>) met system/hidden/readonly-attributen botst tijdens de file-sync; <code>copyFile()</code> gooit en breekt de hele sync af.</td><td>Opgelost sinds v1.27.6: <code>syncDirectory</code> slaat <code>desktop.ini</code>/<code>Thumbs.db</code>/<code>.DS_Store</code> over en het meegestuurde junk-bestand is verwijderd. Update naar ≥1.27.6. Het achtergebleven <code>desktop.ini</code> op de site mag handmatig weg (<code>attrib -s -h</code> dan <code>del</code>) maar is onschadelijk. Sinds v1.28.95 breekt bovendien géén enkel onkopieerbaar bestand de sync meer af — fouten worden verzameld en aan het eind luid gerapporteerd, zodat kritieke bestanden als <code>library.inc</code> nooit half achterblijven.</td></tr>
...
&lt;/div&gt;
    ...
    <p>Sinds v1.29.45 staan de globale functies <code>SendMail()</code>, <code>SendMailNoTemplate()</code> en <code>internal_SendMail()</code> in een <code>function_exists</code>-guard. Niet elke geconverteerde ASP-site spreekt dit 8-parameter dialect: mijnRINO's ASP-mailer had bijvoorbeeld 9 parameters (<code>strFromName</code> tussen from en cc). Zo'n site definieert zijn eigen variant in de eigen bootstrap vóórdat <code>class_mailer.inc</code> laadt; de voorgedefinieerde functie wint dan. Sinds v1.29.43 slaat <code>Email</code> bij verzenden bijlagen over via <code>is_file()</code> i.p.v. <code>file_exists()</code>, zodat ook directory-paden stil worden overgeslagen (gelijk aan <code>lib_FileExists</code> in het ASP-origineel).</p>
