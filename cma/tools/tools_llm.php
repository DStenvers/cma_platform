    ...
    'llamacpp' => [
        ...
        'setup' => [
            'windows' => [
                ...
                ['title' => 'Auto-start via Task Scheduler', 'body' => 'Geen extra install nodig, en <code>schtasks</code> zit in elke Windows-versie — anders dan de PowerShell-cmdlets <code>New-ScheduledTask*</code>, die pas vanaf Server 2012 bestaan en op oudere servers afketsen op <code>Register-ScheduledTask : The parameter is incorrect</code>. Run als SYSTEM, zodat het draait zonder ingelogde gebruiker. Command prompt of PowerShell as Admin:', 'code' => "schtasks /Create /TN \"LlamaServer\" /SC ONSTART /RU \"SYSTEM\" /RL HIGHEST /F /TR \"C:\\llama\\llama-server.exe -m C:\\llama\\models\\$recGGUF --port 8080 --host 0.0.0.0\"\nschtasks /Run /TN \"LlamaServer\""],
                ['title' => 'Herstart na een crash (optioneel)', 'body' => 'Dit is het enige dat <code>schtasks /Create</code> niet met een vlag kan. Zet het er handmatig bij: <span class="cma-tool__strong">Task Scheduler → LlamaServer → Eigenschappen → tabblad Instellingen → "Taak opnieuw starten elke: 1 minuut, tot 5 keer"</span>. Wie het scriptbaar wil: exporteer de taak naar XML, zet <code>&lt;RestartOnFailure&gt;</code> erin en importeer terug — dat werkt ook op oudere servers.', 'code' => "schtasks /Query  /TN \"LlamaServer\" /XML > llamaserver.xml\nschtasks /Create /TN \"LlamaServer\" /XML llamaserver.xml /RU \"SYSTEM\" /F"],
                ['title' => 'Status / stop / verwijderen', 'body' => 'Het verschijnt niet in services.msc — gebruik de Task Scheduler GUI of:', 'code' => "schtasks /Query  /TN \"LlamaServer\" /V /FO LIST\nschtasks /End    /TN \"LlamaServer\"\nschtasks /Delete /TN \"LlamaServer\" /F"],
    ...
    'textgen' => [
        ...
        'setup' => [
            'windows' => [
                ...
                ['title' => 'Auto-start via Task Scheduler', 'body' => 'Built-in Windows, geen extra install. <code>schtasks</code> werkt op elke versie; de <code>New-ScheduledTask*</code>-cmdlets bestaan pas vanaf Server 2012. Command prompt of PowerShell as Admin:', 'code' => "schtasks /Create /TN \"TextGenWebui\" /SC ONSTART /RU \"SYSTEM\" /RL HIGHEST /F /TR \"C:\\pad\\text-generation-webui\\start_windows.bat\"\nschtasks /Run /TN \"TextGenWebui\""],
                ['title' => 'Status / verwijderen', 'body' => '', 'code' => "schtasks /Query  /TN \"TextGenWebui\" /V /FO LIST\nschtasks /Delete /TN \"TextGenWebui\" /F"],
