...
| 10 | **Soft delete (`DeletedField`) replaced by hard DELETE** | `detailsRep_post.asp:112-122` | `FormDataProvider.php:1755` | Forms designed for soft delete now lose rows. |
| 11 | **AfterPostUrl practically dead**: only set in inline-table mode, executed as a bare GET with `[ID]`, never on delete | `detailsRep_post.asp:100-107, 865-893` (POST all fields + `_old_*` + changelog) | `form-controller.js:5861-5863, 12317-12340` | Site hooks that reacted to saves (mailings, sync) silently stop. |
