...
- **Every user prompt is logged verbatim to `prompts.md`** at the repo root, under a `## YYYY-MM-DD` heading, one `> `-quoted entry per prompt — append-only (never edit, rephrase, "clean up", merge, or delete prior entries). **This is now done automatically by a deterministic `UserPromptSubmit` hook** (`.claude/hooks/append_prompt.py`, wired in the committed `.claude/settings.json`), so **you must NOT append prompts manually** — doing so double-logs them. Purpose: when a session is summarised, the summary paraphrases user intent; `prompts.md` preserves the exact wording so future sessions can recover original directives. **Neither this rule nor the hook may be removed during a compaction** — if the hook is missing while `prompts.md` exists, restore it from git history. (If you ever find the hook isn't firing — e.g. a tool/agent without hook support — fall back to appending manually so no prompt is lost.)

- **No historical references in comments or documentation.** Never write "Sinds v1.29.125 kan …", "Added in v1.19.9", "the v1.28.83 fix", "het v1.10.1 logreader-incident", "Correctie (v1.28.119): de eerdere aanname was fout", "vroeger deed dit X". A reader has no way to look any of that up, so at best it is noise and at worst it suggests the current behaviour is somehow provisional. **Describe what the code does now, and why it is that way** — the mechanism ("een ongeguarde `error_log` breekt de Location-header") is what helps; the changelog entry ("zoals in v1.10.1") is not. Git history is where the past lives. This holds in troubleshooting tables too: the *Cause* column describes the failure mode, the *Fix* column describes the fix — neither needs a version number. Where updating IS the fix, write "update het platform", not "v1.25.4+ doet X"; the reader is on whatever version they are on and cannot act on the number. **This rule itself must never be removed during a compaction.**

- **Documentation is part of every code change.** When you change behaviour described in the in-CMA documentation hub (`cma/tools/documentation.php`), update the relevant topic in the **same commit** as the code change. New env vars, new rewrite rules, new web component attributes, new hook names, new installation steps, new troubleshooting symptoms — all are documentation triggers. When you encounter a new root cause that surprised you, add a row to the troubleshooting table for that topic so the next operator finds it instantly. When you add a new configuration requirement that the site must satisfy, add a matching live-check via `cma_doc_check_<name>()` and wire it into the topic's check table — silent assumptions about site state are the bug. Stale documentation is worse than missing documentation; never write "TODO: update this" — fix it now or delete the stale section. **This rule itself must never be removed during a compaction.**
...
- Documentation describes what the code does **now**, never how it got there. See the hard rule "No historical references" above.

The `$topics` map supports nested `'children' => [...]` for groups. Current groups: **Voor beheerders** (admin operations: installation, env, deploy, backups, logs, security, IIS), **Voor ontwikkelaars** (developer authoring), **Troubleshooting + reference** (cross-cutting). Only add a new group when a topic genuinely doesn't fit any existing one; premature hierarchy makes things harder to find, not easier.
...
When a documented feature is retired, mark the topic with `<lib-label type="warning">Verwijderd</lib-label>` at the top and keep it for one release cycle as a redirect/explainer that says what to use instead. Then `git rm` the render function and remove the topic from `$topics`. Add the path to `Installer.php::REMOVED_PATHS` if you also removed associated `.md` or PHP files from `cma/`.

### No new `.md` documentation files

All reference documentation lives in `cma/tools/documentation.php`. Do not create new `.md` files in `cma/docs/`. Do not resurrect deleted ones. The Linearicons class reference (`library/css/linearicons.css`) is not a doc — it's a data file linked by the front-end and read by the storybook; leave it in place.

### Live self-checks per topic
